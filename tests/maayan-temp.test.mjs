...
test('the production Word Forge edition uses the adaptive timer without Maayan branding', async () => {
  const html = await source('word-forge/index.html');

  assert.match(html, /ENGLISH BASIC · WORD FORGE/);
  assert.match(html, /const challengeDurationsMs = \[10000, 7000, 5000\]/);
